...
function updateCurrentCallBeadChar(segment)
    ...
    local currentSelection = AutoTracker:ReadU16(CALL_BEAD_CHARS_MENU_ADDR + 2)
    local readResult = AutoTracker:ReadU16(CALL_BEAD_CHARS_MENU_ADDR + 4 + currentSelection * 2)
...
function updateCallBeadSpells(segment)
    ...
    local ringMenuSize = AutoTracker:ReadU16(CALL_BEAD_SPELLS_MENU_ADDR)
    ...
    for i = 0, 4 do
        ...
        if obj then
            if i > ringMenuSize - 1 then
                ...
                local readResult = AutoTracker:ReadU16(CALL_BEAD_SPELLS_MENU_ADDR + 4 + i * 2)
...
function updateWeaponLevels(segment)
    for k in pairs(WEAPON_LEVELS) do
        ...
        local readResultLow = AutoTracker:ReadU8(WEAPON_LEVELS[k])
        local readResultHigh = AutoTracker:ReadU8(WEAPON_LEVELS[k] + 1)
        if AUTOTRACKER_ENABLE_DEBUG_LOGGING then
            print(
...
function updateDogAttackLevel(segment)
    ...
    local readResultLow = AutoTracker:ReadU8(DOG_ATTACK_LVL_ADDR)
    local readResultHigh = AutoTracker:ReadU8(DOG_ATTACK_LVL_ADDR + 1)
    if AUTOTRACKER_ENABLE_DEBUG_LOGGING then
    ...
    if obj then
        ...
        local high = readResultHigh
        if AUTOTRACKER_ENABLE_DEBUG_LOGGING then
            print(string.format("set weapon level for %s to %s:%00.f", "dog", high, low))
        end
        local str = string.format("%d:%d", high, low)
        obj:SetOverlay(str)
    end
...
function updateAlchemyLevels(segment)
    for i = 1, #ALCHEMY_NAMES do
        ...
        if obj then
            ...
            local readResultLow = AutoTracker:ReadU16(lowAddr)
            local readResultHigh = AutoTracker:ReadU16(highAddr)
            if AUTOTRACKER_ENABLE_DEBUG_LOGGING then
...
function updateMoney(segment)
    for i = 1, #MONEY_TYPES do
        ...
        if obj then
            ...
            local readResultLow16 = AutoTracker:ReadU16(addr)
            local readResultHigh8 = AutoTracker:ReadU8(addr + 2)
            if AUTOTRACKER_ENABLE_DEBUG_LOGGING then
...
function updateBoyExp(segment)
    ...
    local exp
    if obj then
        local readResultLow = AutoTracker:ReadU16(BOY_EXP_ADDR)
        local readResultHigh = AutoTracker:ReadU16(BOY_EXP_ADDR + 2)
    ...
    if objNext then
        local readResultLowNext = AutoTracker:ReadU16(BOY_EXP_NEXT_LVL_ADDR)
        local readResultHighNext = AutoTracker:ReadU16(BOY_EXP_NEXT_LVL_ADDR + 2)
...
function updateBoyLvl(segment)
    ...
    if obj then
        local readResult = AutoTracker:ReadU16(BOY_LVL_ADDR)
...
function updateDogExp(segment)
    ...
    local exp
    if obj then
        local readResultLow = AutoTracker:ReadU16(DOG_EXP_ADDR)
        local readResultHigh = AutoTracker:ReadU16(DOG_EXP_ADDR + 2)
        exp = readResultLow + readResultHigh * 65536
        obj:SetOverlay(string.format("Exp: %i", exp))
    end
    local objNext = Tracker:FindObjectForCode("dog_exp_next_lvl")
    if objNext then
        local readResultLowNext = AutoTracker:ReadU16(DOG_EXP_NEXT_LVL_ADDR)
        local readResultHighNext = AutoTracker:ReadU16(DOG_EXP_NEXT_LVL_ADDR + 2)
        local nextExp = (readResultLowNext + readResultHighNext * 65536) - exp
        objNext:SetOverlay(string.format("to next: %i", nextExp))
    end
end
...
function updateDogLvl(segment)
    ...
    if obj then
        local readResult = AutoTracker:ReadU16(DOG_LVL_ADDR)
        obj:SetOverlay(string.format("Level: %i", readResult))
    end
end
...
function updateEnergyCoreFragments(segment)
    ...
    if obj then
        local readResult = AutoTracker:ReadU16(ENERGY_CORE_FRAGMENTS_ADDR)
        obj.AcquiredCount = readResult
    end
end

function updateSettings(segment)
    local seed = 0
    for i = 0, 7 do
        local val = AutoTracker:ReadU8(SETTINGS_ADDR, i)
        seed = seed | val * 1 << (8 * i)
    end
    local version = ""
    for i = 0, 2 do
        version = version .. _getASCIIChar(SETTINGS_ADDR, 8 + i)
    end
    local flags = AutoTracker:ReadU8(SETTINGS_ADDR, 8 + 3 + 1)
    local settings = ""
    for i = 0, 34 do
        local char = _getASCIIChar(SETTINGS_ADDR, 8 + 3 + 1 + 1 + i)
        if char == string.char(0) then
            break
        end
        settings = settings .. char
    end
    SEED = seed
    EVERMIZER_VERSION = tonumber(version)
    SETTING_FLAGS = flags
    SETTINGS = settings
...
function parseSettings(flags, settings)
    ...
    for char, info in pairs(SETTINGS_MAP) do
        local code = info[1]
...
function _getASCIIChar(addr, offset)
    local val = AutoTracker:ReadU8(addr, offset)
    return string.char(val)
end

function updateRequiredEnergyCoreFragments(segment)
    local val = segment:ReadUInt8(REQUIRED_ENERGY_CORE_FRAGMENTS_ADDR)
    local obj = Tracker:FindObjectForCode("required_energy_core_fragments")
    if obj then
